...
import { formatToCurrency } from '~/screens/Challenge/Challenge.PaymentAward/screens/Payment.SelectOptions.screen';
...
import { useAddProduct, useRemoveProduct } from '~/hooks';
...
function compare(a: ProductType, b: ProductType): number {
...
const PaymentAward: React.FC<Props> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (route.params.data.getChallengeDetail.products) {
      ...
      const filteredAvailableWithoutSubscribeElements =
        filteredAvailableElements.filter(
          (product) => product.allow_buy_without_subscription === true,
        );

      const filterAvailablePerDate =
        filteredAvailableWithoutSubscribeElements.filter((product) =>
          product.date_initial && product.date_end
            ? isPast(new Date(product.date_initial)) &&
              (product.date_end
                ? isFuture(new Date(product.date_end))
                : isFuture(
                    new Date(route.params.data.getChallengeDetail.end_date),
                  ))
            : true,
        );
      ...
      if (route.params.firstElementId) {
  ...
  const handlePricesData = (prices: ProductVariationPriceType[]) => {
    const actualPrice = prices
      ...
      .sort((a, b) => new Date(b.date_initial) - new Date(a.date_initial));

    if (actualPrice && actualPrice.length > 0) {
      return actualPrice[0].value;
    }
  ...
  //         </TouchableOpacity>
  //         <TitleText>Escolha de Produtos</TitleText>
  //         <View style={{ width: 20 }} />
  //       </View>
  //
  //       <ScrollView
  ...
  //                       storeAditionalRequests.length >= awardOptions &&
  //                       storeAditionalRequests[awardOptions] &&
  //                       storeAditionalRequests[awardOptions].response
  //                         ? storeAditionalRequests[awardOptions].response
  //                         : ''
  //                     }
  //                     editable={verifiyEditable()}
  //                     onChangeText={(e) => {
  //                       setSelectedProducts({
  //                         index: awardOptions,
  //                         product: {
  ...
  //               storeAditionalRequests[awardOptions] &&
  //               storeAditionalRequests[awardOptions].picker_index !==
  //                 undefined &&
  //               products[awardOptions].variations[
  //                 storeAditionalRequests[awardOptions].picker_index
  //               ].images?.length > 0 ? (
  ...
  //             storeAditionalRequests.length >= 0 &&
  //             storeAditionalRequests[awardOptions] &&
  //             storeAditionalRequests[awardOptions].picker_index !== undefined &&
  //             products[awardOptions].variations[
  //               storeAditionalRequests[awardOptions].picker_index
  //             ].images?.length > 0 ? (
  ...
  //             QUANTIDADE:{' '}
  //             {storeAditionalRequests && storeAditionalRequests[awardOptions]
  //               ? storeAditionalRequests[awardOptions].quantity
  //               : 1}{' '}
  //             unidade{' '}
  //             {storeAditionalRequests &&
  //             storeAditionalRequests[awardOptions] &&
  ...
  //             {storeAditionalRequests !== undefined &&
  //             storeAditionalRequests !== null &&
  //             storeAditionalRequests.length >= awardOptions &&
  //             storeAditionalRequests[awardOptions] !== undefined &&
  //             products[awardOptions].variations[
  //               storeAditionalRequests[awardOptions]?.picker_index
  //             ].text
  //               ? products[awardOptions].variations[
  ...
  //                 {!Number.isInteger(totalAddonPrice)
  //                   ? String(Number(totalAddonPrice).toFixed(2)).replace(
  //                       '.',
  //                       ',',
  //                     )
  //                   : totalAddonPrice}
  ...
  //                 if (
  //                   !totalAddonPrice &&
  //                   products.length - 1 === awardOptions
  //                 ) {
  //                   Alert.alert(
  //                     'Atenção',
  //                     'Para poder continuar você deve selecionar algum item.',
  //                     [
  //                       {
  //                         onPress: () => setAwardOptionStep(0),
  //                         text: 'Ok, vou continuar comprando',
  //                       },
  //                       {
  ...
  //                     index: awardOptions,
  //                   });
  //                   removeElement({ index: awardOptions });
  ...
  //                     });
  //                   } else {
  //                     navigation.push('selectAddress', ...route.params);
  //                   }
  //                 }
  //               }}
  //             >
  ...
  //                   !storeAditionalRequests ||
  //                   !storeAditionalRequests[awardOptions] ||
  //                   storeAditionalRequests[awardOptions] === null
  //                     ? '#A1A8B1'
  //                     : '#0564FF',
  //                 borderRadius: 24,
  //                 alignItems: 'center',
